...
def second():
    ...
    for filename in os.listdir(folder):
        ...
        print(good_color + "Deleted " + filename, '\n')
        try:
            ...
            time.sleep(1)
            junk_removed = len(folder)
            print(good_color + 'Total junk files deleted in the first folder:', junk_removed, '\n')
            time.sleep(2)
            third()
...
def third():
    ...
    for filename in os.listdir(folder):
        ...
        try:
            ...
            time.sleep(1)
            junk_removed = len(folder)
            print(good_color + 'Total junk files deleted in the second folder:', junk_removed, '\n')
            time.sleep(2)
            end()
